...
class RagAgentService:
    ...
    def _build_system_prompt(self) -> str:
        ...
        return dedent("""
            ...
            日志查询指南:
            - 查询日志时，默认使用过去 1 小时作为时间范围
            - 如果 1 小时内无结果，自动扩大到 6 小时、24 小时
            - 未指定日志级别时，优先查询 ERROR 和 WARN 级别
    ...
    def get_session_history(self, session_id: str) -> list:
        ...
        try:
            ...
            # 新会话或空会话没有检查点是正常情况，不需要 ERROR 级别
            logger.info(f"获取会话历史: {session_id}, 无存储记录（新会话或已过期）")
            return []
